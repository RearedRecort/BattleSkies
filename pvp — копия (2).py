...
class PvpView(arcade.View):
    def __init__(self, is_owner, game, x, y, team):
        ...
        # Кэш никнеймов: {player_id: nickname}
        self.nick_cache = {}
        # Текущие чужие игроки: {player_id: {'world_x', 'world_y', 'team', 'name'}}
        self.other_players = {}

        # Batch для оптимизированной отрисовки никнеймов (pyglet)
        self.name_batch = pyglet.graphics.Batch()
        # Метки: {player_id: pyglet.text.Label}
        self.name_labels = {}
        self.kills_friends = 0
        self.kills_enemy = 0

    def _world_to_screen(self, x, y):
        # Преобразование координат мира -> экран (учитываем позицию камеры и zoom).
        try:
    ...
    def on_update(self, delta_time):
        try:
            ...
            current_pids = set()
            for lst in lstk:
                # Поля: id, player, game, plane, team, x, y, v, angle, hp
                if lst[1] != vars.id:
                    ...
                    # Позиция над самолётом (мировые координаты)
                    ...
                    # Создаем или обновляем pyglet.Label в батче (позицию обновим ниже)
                    ...
                    # Наш игрок: обновляем локально и в БД
                    self.player.update(delta_time)
                    if self.up:
                        self.player.vx *= 1.25
                        self.player.vy *= 1.25
                    if self.down:
                        # делаем нецелочисленное деление, чтобы скорость была дробной
                        self.player.vx /= 1.25
                        self.player.vy /= 1.25
                    if self.left:
            ...
            # Обновляем экранные позиции меток — так имена будут следовать за самолётом
            ...
            if self.is_owner:
                self.transfer_host()
            lose_view = LoseView(self.kills_friends,
                                 self.kills_enemy, GLOBAL_CLOCK.time_since(0.0), "Потеряно соединение")
            self.manager.clear()
    ...
    def on_draw(self):
        ...
        # GUI / экранные элементы: батч меток рисуем в экранных координатах
        self.gui_camera.use()
        try:
            self.name_batch.draw()
        except Exception:
            # Если что-то пошло не так с batсh, молча продолжаем
            pass
    ...
    def disconnect(self):
        ...
        vars.con.commit()
        lose_view = LoseView(self.kills_friends,
                             self.kills_enemy, GLOBAL_CLOCK.time_since(0.0), "Выход из игры")
        self.manager.clear()
        self.window.show_view(lose_view)
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        cur.execute("INSERT INTO `shouting` (`game`, `x`, `y`, `vx`, `vy`, `damage`, `time`, `player`"
                    + f") VALUES ({game}, {mx}, {my}, {vx}, {vy}, {damage}, {time}, {sender})")
        vars.con.commit()
    ...
    def transfer_host(self):
        cur = vars.con.cursor()
        id = cur.execute("SELECT `player` FROM `session` WHERE `game` ="
                    + f" {self.game} AND `team` = 0 AND `player` <> {vars.id}").fetchall()[0][0]
    ...
    def dead(self):
        ...
        vars.con.commit()
        lose_view = LoseView(self.kills_friends,
                             self.kills_enemy, GLOBAL_CLOCK.time_since(0.0), "Ваш самолёт уничтожен")
        self.manager.clear()
        self.window.show_view(lose_view)
